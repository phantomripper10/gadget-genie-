...
fetch("/api/status").then((r) => r.json()).then((s) => {
  ...
  pill.textContent = s.ai ? `🟢 AI: ${s.model}` : "🔌 Demo mode";
...
const CAT_LABEL = { practical: "🛠️ Practical", fun: "🎮 Fun Toy" };
...
const MAT_LABEL = {
  cardboard: "📦 cardboard", plastic: "🧴 bottles & plastic", wood: "🪵 scrap wood",
...
function renderGallery() {
  ...
  if (!items.length) {
    el.innerHTML = `<div class="gallery-empty">No ready-made match — hit <b>⚡ Invent it!</b> and the AI will design it for you${filterMat !== "any" ? " using just your " + MAT_LABEL[filterMat] : ""}!</div>`;
    return;
  }
  ...
  el.innerHTML = items.map((g) => {
    ...
    <button class="gcard ${locked ? "gcard-locked" : ""}" onclick="${locked ? "openParentModal()" : `openProject('${g.id}')`}">
      <div class="gcard-emoji">${locked ? "🔒" : g.emoji}</div>
      <div class="gcard-body">
        <b>${escapeHtml(g.name)}</b>
        <span class="gcard-tag">${locked ? "Advanced build — ask a grown-up to unlock Parent Mode!" : escapeHtml(g.tagline)}</span>
        <div class="gcard-pills">
          <span class="gpill gpill-cat">${CAT_LABEL[g.category] || "🔧"}</span>
...
function renderSupervision(g) {
  ...
  el.innerHTML = `<div class="supervision">
      <b>🛡️ Parent supervision checklist</b>
...
function renderTab(tab) {
  ...
  if (tab === "parts") {
    const rows = (g.parts || []).map((p) => {
      ...
      return `<tr>
        <td>${escapeHtml(p.item)}${p.sustainable ? `<span class="sust-alt">♻️ Free option: ${escapeHtml(p.sustainable)}</span>` : ""}</td>
        <td>${escapeHtml(p.qty)}</td>
        <td class="cost">${cost === 0 ? "FREE" : "$" + cost.toFixed(2)}</td>
        <td>${p.buy ? `<a class="buy-link" target="_blank" rel="noopener" href="https://www.google.com/search?tbm=shop&q=${encodeURIComponent(p.buy)}">🛒 Buy</a>` : "♻️"}</td>
      </tr>`;
    }).join("");
    const total = (g.parts || []).reduce((s, p) => s + (Number(p.cost) || 0), 0);
...
${(g.parts || []).map((p) => `<tr><td>${escapeHtml(p.item)}${p.sustainable ? `<br><small>♻️ Free option: ${escapeHtml(p.sustainable)}</small>` : ""}</td><td>${escapeHtml(p.qty)}</td><td>${Number(p.cost) ? "$" + Number(p.cost).toFixed(2) : "FREE"}</td></tr>`).join("")}
</table>
